...
extern "C" {
    ...
    const char* compute_sha512(const char* input) {
        static std::string hash_output;  // Static to persist after function call
...
//     // Validate input
//     if (input.empty()) {
//         std::cerr << "Error: Empty input provided. Please enter a valid string." << std::endl;
//         return 1;
//     }

//     // Compute the SHA-512 hash
//     SHA512 sha512;
//     std::string hash = sha512.hash(input);
